...
public class AssembleThread extends Thread {
    ...
    private void tick() {
        ...
        new NeptuneRunnable() {
            @Override
            public void run() {
                for (Player player : assemble.getPlugin().getServer().getOnlinePlayers()) {
                    try {
                        AssembleBoard board = assemble.getBoards().get(player.getUniqueId());
                        ...
                        List<String> newLines = assemble.getAdapter().getLines(player);
                        ...
                        if (newLines == null || newLines.isEmpty()) {
                            ...
                            if (!assemble.getAssembleStyle().isDescending()) {
                            ...
                            int cache = assemble.getAssembleStyle().getStartNumber();
                            for (int i = 0; i < newLines.size(); i++) {
                                ...
                                entry.setup();
                                entry.send(
                                        assemble.getAssembleStyle().isDescending() ? cache-- : cache++
                                );
                            }
                        }
